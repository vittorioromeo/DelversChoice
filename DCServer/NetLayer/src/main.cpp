...
#define NL_IMPL_DEFINE_PCKT_TEMPLATE_LIST_FOR_IMPL(mIdx, mData, mArg) \
    VRM_PP_TPL_ELEM(mArg, 0) VRM_PP_COMMA_IF(mIdx)
...
using ClientPackets = nl::PcktSet<nl::PcktIDAuto<AuthRequest>,
                                  nl::PcktIDFixed<100, ImportantPacket>>;

using ServerPackets =
    nl::PcktSet<nl::PcktIDAuto<AuthSuccess>, nl::PcktIDAuto<AuthFailure>>;
...
{
#define LOSIZE(x) \
    ssvu::lo("sizeof("s + VRM_PP_TOSTR(x) + ")") << sizeof(x) << "\n"
    ...
    SSVU_ASSERT_STATIC_NM(
        ssvu::isSame<ClientPackets::PcktType<0>, AuthRequest>());
    SSVU_ASSERT_STATIC_NM(
        ssvu::isSame<ClientPackets::PcktType<100>, ImportantPacket>());

    SSVU_ASSERT_STATIC_NM(
        ssvu::isSame<ServerPackets::PcktType<0>, AuthSuccess>());
    SSVU_ASSERT_STATIC_NM(
        ssvu::isSame<ServerPackets::PcktType<1>, AuthFailure>());
