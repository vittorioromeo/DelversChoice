...
namespace ssvs
{
	namespace BTR
	{
		namespace Impl
		{
			class BTRChunk
			{
				template<typename> friend struct BTR::Impl::EffectHelper;
				...
				public:
					...
					auto& in();
					auto& in(BTR::Ptr<BTRChunk>&);
					template<typename T> auto& in(T&& mStr);
					template<typename T> auto& in(BTR::Ptr<BTRChunk>&, T&& mStr);

					// Make children and stay on the same level
					auto& mk(BTR::Ptr<BTRChunk>&);
					template<typename T> auto& mk(T&& mStr);
					template<typename T> auto& mk(BTR::Ptr<BTRChunk>&, T&& mStr);

					// Add effects
					auto& eff(const sf::Color&);
					template<typename T, typename... TArgs> auto& eff(TArgs&&...);
					template<typename T, typename... TArgs> auto& eff(BTR::Ptr<T>&, TArgs&&...);
