...
namespace ggj
{
	...
	template<typename TW = float> class WeightedChance
	{
		...
		public:
			inline void reserve(ssvu::SizeT mX)
			{
				weights.reserve(mX);
			}
	...
	template<typename TW = float, typename... TFns> class WeightedSwitch
	{
		private:
	...
	/*template<typename TW = float, typename... TArgs> inline auto makeWeightedSwitch(TArgs&&... mArgs)
	{
