...
namespace ggj
{
    struct GameSession
    {
        // enum class State : int{Playing = 0, Dead = 1, Menu = 2, Settings =
        // 3};
        ...
        int roomNumber{0};
        Creature player;
        std::array<ssvu::UPtr<Choice>, Constants::maxChoices> choices,
            nextChoices;
        float timer;
        ...
        inline void restart()
        {
            ...
            wcDrop = mkWeightedChance(gd.dropChanceIE, // 0
                gd.dropChanceWeapon,                   // 1
                gd.dropChanceArmor                     // 2
                );

            wcChoice = mkWeightedChance(gd.choiceChanceCreature, // 0
        ...
        inline void startDrops(ItemDrops* mID) { currentDrops = mID; }
        inline void endDrops() { currentDrops = nullptr; }

        inline void refreshProfileData() { pd = ProfileData::fromFile(); }
        ...
        {
            if(mode == Mode::Official || mode == Mode::Beginner)
                timer = ssvu::getSecondsToFT(10);
            else if(mode == Mode::Hardcore)
                timer = ssvu::getSecondsToFT(6);
        }
        ...
        inline InstantEffect generateInstantEffect(
            InstantEffect::Stat mStat, InstantEffect::Type mType, float mValue)
        {
            ...
            switch(mStat)
            {
                ...
                case InstantEffect::Stat::SATK:
                    ieValue = mValue / gd.valueATK;
                    break;
                case InstantEffect::Stat::SDEF:
                    ieValue = mValue / gd.valueDEF;
                    break;
            }
            ...
            ssvu::lo() << result.getStrType() << result.getStrStat()
                       << ", value: " << mValue << "\n";
        ...
        inline auto getShuffledStats()
        {
            return mkShuffledVector<InstantEffect::Stat>(
                InstantEffect::Stat::SHPS, InstantEffect::Stat::SATK,
        ...
        inline auto addIEs(DropIE& dIE)
        {
            ...
            if(ssvu::getRndI(0, 50) > 25)
                std::swap(splitPositive, splitNegative);
            ...
            dIE.addIE(generateInstantEffect(
                ss[0], InstantEffect::Type::Add, valueTotal * splitPositive));
            dIE.addIE(generateInstantEffect(
                ss[1], InstantEffect::Type::Sub, valueTotal * splitNegative));
        ...
        inline ItemDrops generateDrops()
        {
            ...
            if(ssvu::getRndR(0.f, 1.f) < gd.multipleDropChance)
                result.drops[indices[1]] = ssvu::mv(generateRndDrop());
            if(ssvu::getRndR(0.f, 1.f) < gd.multipleDropChance)
                result.drops[indices[2]] = ssvu::mv(generateRndDrop());
        ...
        inline Creature generateCreature(bool mEnemy)
        {
            ...
            auto bucket(ssvu::mkArray(
                ssvu::getRndR(0, 3.f), ssvu::getRndR(0, 3.f), 0.f, 3.f));
        ...
        inline void generateChoices()
        {
            ...
            if(roomNumber < gd.section1)
                choiceCount = gd.section0ChoiceCount;
            else if(roomNumber < gd.section2)
                choiceCount = gd.section1ChoiceCount;
            else if(roomNumber < gd.section3)
                choiceCount = gd.section2ChoiceCount;
            else if(roomNumber < gd.section4)
                choiceCount = gd.section3ChoiceCount;
            else
                choiceCount = gd.section4ChoiceCount;
            ...
            for(int i{0}; i < choiceCount; ++i)
            {
                ...
                switch(wcChoice.get())
                {
                    ...
                    case 2:
                        choices[idx] = generateChoiceMultipleDrop(idx);
                        break;
                }
            }
        }
