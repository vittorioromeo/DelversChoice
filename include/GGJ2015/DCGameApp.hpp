...
namespace ggj
{
    ...
    public:
        template <typename T>
        inline MenuChoice(std::string mStr, T&& mFn)
    ...
    public:
        ...
        inline void addChoice(SizeT mIdx, TArgs&&... mArgs)
        {
    ...
    template <typename TF>
    void Menu::forCurrentChoices(TF&& mFn)
    {
    ...
    private:
        ...
        ssvs::BitmapText txtTimer{mkTxtOBBig()}, txtRoom{mkTxtOBBig()},
            txtDeath{mkTxtOBBig()}, txtLog{mkTxtOBSmall()},
            txtMode{mkTxtOBSmall()};

        ssvs::BitmapTextRich txtCredits{*getAssets().fontObStroked},
            txtRestart{*getAssets().fontObStroked},
            txtMenu{*getAssets().fontObStroked},
            txtScores{*getAssets().fontObStroked};
        BTR::PtrChunk bpstrRoom, bpstrMode, sScoreName, sScoreBeginner,
            sScoreOfficial, sScoreHardcore, sScorePlayedGames, sScorePlayedTime;
        ...
        inline void initInput()
        {
            ...
            gState.addInput({{IK::Escape}},
                [this](FT)
                {
                    if(state != State::Menu) gotoMenu();
                },
                IT::Once);
            ...
            */
            ...
            gState.addInput({{IK::Num1}},
                [this](FT)
                {
                    executeChoice(0);
                },
                IT::Once);
            gState.addInput({{IK::Num2}},
                [this](FT)
                {
                    executeChoice(1);
                },
                IT::Once);
            gState.addInput({{IK::Num3}},
                [this](FT)
                {
                    executeChoice(2);
                },
                IT::Once);
            gState.addInput({{IK::Num4}},
                [this](FT)
                {
                    executeChoice(3);
                },
                IT::Once);
            gState.addInput({{IK::Num5}},
                [this](FT)
                {
                    executeChoice(4);
                },
        ...
        inline const auto& getModeStrArray()
        {
            static auto array(ssvu::mkArray(
                "Beginner mode", "Official mode", "Hardcore mode"));
        ...
        {
            stMain = menu.mkState();
            stMain->addChoice(0, "Play game", [this]
                {
                    goMenuState(stPlay);
                });
            stMain->addChoice(1, "Settings", [this]
                {
                    goMenuState(stSettings);
                });
            stMain->addChoice(4, "Exit game", [this]
                {
                    stop();
                });

            stPlay = menu.mkState();
            stPlay->addChoice(0, getModeStrArray()[0], [this]
                {
                    gotoGame(GameSession::Mode::Beginner);
                });
            stPlay->addChoice(1, getModeStrArray()[1], [this]
                {
                    gotoGame(GameSession::Mode::Official);
                });
            stPlay->addChoice(2, getModeStrArray()[2], [this]
                {
                    gotoGame(GameSession::Mode::Hardcore);
                });
            stPlay->addChoice(4, "Go back", [this]
                {
                    goMenuBack();
                });
        ...
        inline void executeChoicePlaying(int mI)
        {
            ...
            if(gs.currentDrops == nullptr)
            {
                ...
                if(gs.currentDrops == nullptr) gs.refreshChoices();
            }
            else
            {
                if(mI == 0)
        ...
        inline void executeChoice(int mI)
        {
            ...
            else if(gs.state == GameSession::State::Playing)
                executeChoicePlaying(mI);
            else if(gs.state == GameSession::State::Dead)
                executeChoiceDead(mI);
        }
        ...
        inline void updatePlaying(FT mFT)
        {
            ...
            for(auto& c : gs.choices)
                if(c != nullptr) c->update(mFT);
            ...
            else
            {
                ...
                auto gts(
                    intt >= 10 ? ssvu::toStr(intt) : "0" + ssvu::toStr(intt));
        ...
        inline void update(FT mFT)
        {
            ...
            if(gs.shake > 0)
            {
                ...
                gameCamera.setCenter(
                    oldPos + Vec2f{ssvu::getRndR(-shake, shake + 0.1f),
                                 ssvu::getRndR(-shake, shake + 0.1f)});
            }
            else
            {
        ...
        inline void drawPlaying()
        {
            ...
            if(gs.currentDrops != nullptr)
            {
                ...
                for(auto i(0u); i < slotChoices.size(); ++i)
                {
                    ...
                    {
                        gs.currentDrops->drops[i - 1]->draw(
                            gameWindow, sc.shape.getPosition(), sc.getCenter());
            ...
            else
            {
                for(auto i(0u); i < slotChoices.size(); ++i)
                {
                    ...
                    sc.txtNum.setString(
                        gc == nullptr ? "" : ssvu::toStr(i + 1));
                    sc.txtStr.setString(
                        gc == nullptr ? "Blocked" : gc->getChoiceStr());
                    sc.sprite.setTexture(gc == nullptr
                                             ? *getAssets().slotChoiceBlocked
                                             : *getAssets().slotChoice);
                    sc.update();
            ...
            csdPlayer.draw(
                gs.player, gameWindow, Vec2f{10, 175}, Vec2f{0.f, 0.f});
        }
        ...
        inline void drawDead()
        {
            ...
            txtDeath.setColor(sf::Color(255, 255, 255,
                255 - ssvu::toNum<unsigned char>(gs.deathTextTime)));
        }
            ...
            menu.forCurrentChoices([this](auto mIdx, auto& mC)
                {
                    ...
                    {
                        txtMenu.eff<BTR::PulseDef>(sfc::Red)
                            .in(ssvu::toStr(mIdx + 1))
                            .in(". ")
                            .eff(sfc::White)
                            .in(mC->getStr())
                            .in("\n");
    ...
    public:
        ...
        inline GameApp(ssvs::GameWindow& mGameWindow)
            : Boilerplate::App{mGameWindow}
        {
            tr.setAlign(ssvs::TextAlign::Center);
            tr.in("Testing rich text...\n")
                .eff<BTR::Leading>(-4)
                .in("Here it goes...")
            ...
            txtCredits.eff<BTR::Tracking>(-3)
                .eff<BTR::PulseDef>(sfc::White)
                .in("Global Game Jam 2015\n")
                .eff(sfc::White)
                .in("Developer: ")
                .eff(sfc::Red)
                .in("Vittorio Romeo\n")
                ...
                .in("Additional help: ")
                .eff(sfc::Yellow)
                .in("Sergio Zavettieri\n")
                .eff(sfc::Cyan)
                .in("http://vittorioromeo.info\nhttp://nicolabombaci.com");

            txtRestart.eff<BTR::Tracking>(-3)
                .eff(sfc::White)
                .in("Press ")
                .eff<BTR::PulseDef>(sfc::Red)
                .in("1 ")
